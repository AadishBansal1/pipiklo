        ...
        {(['pending', 'approved', 'rejected', 'all'] as const).map((tab) => (
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium capitalize transition-colors border-b-2 -mb-px ${
              filter === tab ? 'border-brand-500 text-brand-600 dark:text-brand-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'
            }`}
          >
            {tab}
          ...
          {filtered.map((item) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">by {item.creatorName} · {item.category}</p>
                <div className="flex flex-wrap gap-1 mt-2">
                  {item.tags.slice(0, 3).map((tag) => (
                    <span key={tag} className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-[10px] text-gray-500">#{tag}</span>
                  ))}
                </div>
        ...
        {rejectModal && (
          ...
          >
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }}
              ...
              className="w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl"
            >
              <h3 className="font-bold text-gray-900 dark:text-white mb-1">Reject Submission</h3>
              <p className="text-sm text-gray-500 mb-4">Provide a reason so the creator can improve.</p>
              <textarea rows={3} value={rejectReason} onChange={(e) => setRejectReason(e.target.value)}
                placeholder="e.g. Poor quality, missing description, wrong category…"
                className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 resize-none"
              />
              <div className="flex gap-3 mt-4">
                <button onClick={() => setRejectModal(null)} className="flex-1 py-2.5 rounded-xl border text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Cancel</button>
